...
import { updateLayout, setViewMode } from "../store/layoutSlice";
...
function Dashboard() {
  const { metrics, setMetrics, specs, setSpecs } = useDashboardStore();
  const { layouts, viewMode } = useSelector((state: RootState) => state.layout);
  const dispatch = useDispatch();

  const [time, setTime] = useState(new Date());
  const [clickThroughActive, setClickThroughActive] = useState(false);
  const [currentBreakpoint, setCurrentBreakpoint] = useState("lg");
  const [cpuBars] = useState(() =>
      ...
      <header className="flex items-center justify-between mb-8 pointer-events-auto">
        ...
        <div className="flex items-center gap-3 bg-black/40 backdrop-blur-md p-1.5 rounded-full border border-white/5 shadow-xl">
          <motion.button
            ...
            onClick={() =>
              dispatch(setViewMode(viewMode === "wide" ? "compact" : "wide"))
            }
          >
      ...
      >
        <div key="clock">
          <WidgetWrapper
            size="1x1"
            title="Local Time"
            className="bg-linear-to-br from-indigo-500/10 to-purple-500/10 h-full"
          >
            ...
            <div className="flex items-end justify-between gap-6 h-full">
              ...
              <div className="relative h-20 w-32 overflow-hidden rounded-xl bg-slate-900/50 border border-white/5">
                <div className="absolute inset-0 flex items-end gap-1 p-1">
                  {cpuBars.map((randomVal, i) => (
                    <motion.div
                      key={i}
                      className="flex-1 bg-indigo-500/40 rounded-sm"
                      animate={{
                        height: `${Math.max(10, randomVal * cpuUsage)}%`,
                        opacity: 0.5 + i / 24,
                      }}
                      transition={{
                        type: "spring",
          ...
          >
            <div className="flex flex-col justify-center h-full gap-4">
              ...
              <div className="relative h-2 overflow-hidden rounded-full bg-slate-800">
                <motion.div
                  className="absolute inset-y-0 left-0 rounded-full bg-linear-to-r from-blue-500 via-indigo-500 to-purple-500"
                  initial={{ width: 0 }}
        ...
        <div key="temp">
          <WidgetWrapper size="1x1" title="Thermals" className="h-full">
            {hottestProbe ? (
              <div className="flex h-full flex-col justify-center items-center relative">
                ...
                <div
                  className={`text-3xl font-bold ${hottestProbe.temperature > 80 ? "text-red-400" : "text-orange-300"}`}
                >
        ...
        <div key="specs">
          <WidgetWrapper size="2x2" title="System Specs" className="h-full">
            {specs ? (
